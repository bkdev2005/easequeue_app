...
class _ProfileWidgetState extends State<ProfileWidget> {
  ...
  void initState() {
    ...
    _model.yourNameTextController1 ??=
        TextEditingController(text: FFAppState().user['full_name'] ?? '');
    _model.yourNameFocusNode1 ??= FocusNode();

    _model.mobileTextController2 ??=
    ...
    genderSelected = (FFAppState().user['gender'] == 'Male' ||
        ...
        ? 1
        : (FFAppState().user['gender'] == 'Female' ||
                FFAppState().user['gender'] == 2)
            ? 2
            : 0;
  }
                              ...
                              if (value != null) {
                                profileApi(value, 'user/profile-picture')
                                  ...
                                  if (value != null) {
                                    ...
                                    setState(() {
                                      imageUrl =
                                          "https://staging-api.easequeue.com/shared/${value['image_url']}";
                                    });
              ...
              child: FFButtonWidget(
                onPressed: () async {
                  ...
                  if (name.isNotEmpty) {
                    ...
                    if (callApi != null) {
                      fetchData('user/${FFAppState().userId}', context)
                          ?.then((value) {
                        log('value: ${getJsonField(value, r'''$.data''')}');
                        setState(() {
                          FFAppState().user =
                              getJsonField(value, r'''$.data''');
                        });
                      });
